# Development manual for AnancyIO
This guide will show you how to setup a local development environment for AnancyIO in a VS Code compatible IDE, including proper debugger.
...
> [!NOTE]
> - AnancyIO runs in a Docker container, this simplifies installation and ensures unified environment and behavior across systems.
...
- AnancyIO is available on GitHub [github.com/anancyioai/anancyio](https://github.com/anancyioai/anancyio).
- You can download the files using a browser and extract or run `git clone https://github.com/anancyioai/anancyio` in your desired directory.

> [!NOTE]
> In my case, I used `cd ~/Desktop` and `git clone https://github.com/anancyioai/anancyio`, so my project folder is `~/Desktop/anancyio`.
...
- AnancyIO comes with `.vscode` folder that contains basic setup, recommended extensions, and debugger profiles. These will help us a lot.

1. Open your IDE and open the project folder using `File > Open Folder` and select your folder, in my case `~/Desktop/anancyio`.
...
## Step 4: Run AnancyIO in the IDE
Great work! Now you should be able to run AnancyIO from your IDE including real-time debugging.
...
After inserting my API key in settings, my AnancyIO instance works. I can send a simple message and get a response.
...
## Step 5: Run another instance of AnancyIO in Docker
- Some parts of A_IO require standardized linux environment, additional web services and preinstalled binaries that would be unneccessarily complex to set up in a local environment.
- To make development easier, we can use existing A_IO instance in docker and forward some requests to be executed there using SSH and RFC (Remote Function Call).

1. Pull the docker image `anancyioai/anancyio` from Docker Hub and run it with a web port (`80`) mapped and SSH port (`22`) mapped.
If you want, you can also map the `/a_io` folder to our local project folder as well, this way we can update our local instance and the docker instance at the same time.
This is how it looks in my example: port `80` is mapped to `8880` on the host and `22` to `8822`, `/a_io` folder mapped to `/Users/frdel/Desktop/anancyio`:
...
You have successfully set up a complete AnancyIO development environment! You now have:
...
- ✅ The ability to develop, debug, and test AnancyIO features seamlessly

You're now ready to contribute to AnancyIO, create custom extensions, or modify the framework to suit your needs. Happy coding! 💻✨
...
- Navigate to your project root in the terminal and run `docker build -f DockerfileLocal -t anancyio-local --build-arg CACHE_DATE=$(date +%Y-%m-%d:%H:%M:%S) .`
