# AnancyIO Backup/Restore Backend Specification

## Overview
This specification defines the backend implementation for AnancyIO's backup and restore functionality, providing users with the ability to backup and restore their AnancyIO configurations, data, and custom files using glob pattern-based selection. The backup functionality is implemented as a dedicated "backup" tab in the settings interface for easy access and organization.
...
- Integrates with AnancyIO's established error handling and toast notification system
...
backup_section: SettingsSection = {
    ...
    "description": "Backup and restore AnancyIO data and configurations using glob pattern-based file selection.",
    "fields": [
        {
            "id": "backup_create",
            "title": "Create Backup",
...
def _get_default_patterns(self) -> str:
    ...
    agent_root = self.anancy_io_root.rstrip('/')
    user_home = self.user_home.rstrip('/')

    return f"""# AnancyIO Knowledge (excluding defaults)
{agent_root}/knowledge/**
!{agent_root}/knowledge/default/**

# AnancyIO Instruments (excluding defaults)
...
/a_io/knowledge/**
!/a_io/knowledge/default/**
/root/**
!/root/.*/**
!/root/.*

# Host environment
/home/rafael/a_io/data/knowledge/**
!/home/rafael/a_io/data/knowledge/default/**
/home/rafael/**
!/home/rafael/.*/**
!/home/rafael/.*
```

> **⚠️ CRITICAL FILE NOTICE**: The `{agent_root}/.env` file contains essential configuration including API keys, model settings, and runtime parameters. This file is **REQUIRED** for AnancyIO to function properly and should always be included in backups alongside `settings.json`. Without this file, restored AnancyIO instances will not have access to configured language models or external services.
...
class BackupCreate(ApiHandler):
    ...
    async def process(self, input: dict, request: Request) -> dict | Response:
        patterns = input.get("patterns", "")
        include_hidden = input.get("include_hidden", False)
        backup_name = input.get("backup_name", "anancyio-backup")
    ...
    async def process(self, input: dict, request: Request) -> dict | Response:
        ...
        try:
            ...
            return {
                ...
                "anancy_io_version": metadata.get("anancy_io_version", "unknown"),
...
The BackupService leverages AnancyIO's existing file operation helpers which already support RFC (Remote Function Call) routing for development mode. This ensures seamless operation whether running in direct mode or with container isolation.
...
class BackupService:
    """Core backup and restore service for AnancyIO"""

    def __init__(self):
        self.anancy_io_version = self._get_anancy_io_version()
        self.anancy_io_root = files.get_abs_path("")  # Resolved AnancyIO root
    ...
    def _get_anancy_io_version(self) -> str:
        """Get current AnancyIO version"""
        try:
    ...
    async def _get_environment_info(self) -> Dict[str, Any]:
        ...
        try:
            return {
                ...
                "anancy_io_root": files.get_abs_path(""),
    ...
    async def create_backup(self, patterns: str, include_hidden: bool = False, backup_name: str = "anancyio-backup") -> str:
        ...
        try:
            with zipfile.ZipFile(zip_path, 'w', zipfile.ZIP_DEFLATED) as zipf:
                ...
                metadata = {
                    ...
                    "anancy_io_version": self.anancy_io_version,
    ...
    def create_backup_with_progress(self, patterns: str, include_hidden: bool = False, backup_name: str = "anancyio-backup"):
...
#### AnancyIO Internal Dependencies
The backup system requires these AnancyIO helper modules:
...
#### Integration with AnancyIO Error System
The backup system integrates with AnancyIO's existing error handling infrastructure:

```python
from python.helpers.errors import format_error
from python.helpers.print_style import PrintStyle

# Follow AnancyIO's error handling patterns
try:
...
- Restrict backups to predefined base directories (/a_io, /root)
...
BACKUP_CONFIG = {
    ...
    "allowed_base_paths": ["/a_io", "/root"]
}
```

#### Future Integration Opportunities
**Task Scheduler Integration:**
AnancyIO's existing task scheduler could be extended to support automated backups:
...
The backup system uses the same version detection method as AnancyIO's main UI:
...
```json
{
  ...
  "include_patterns": [
    ...
    "/a_io/memory/**",
    "/a_io/.env",
    "/a_io/tmp/settings.json"
  ],
  "exclude_patterns": [
    "/a_io/knowledge/default/**",
    "/a_io/instruments/default/**",
    "/a_io/memory/embeddings/**"
  ],
...
- **Environment Details**: User, timezone, working directory, runtime mode, AnancyIO root path
...
- **Compatibility Data**: AnancyIO version and environment for restoration validation
...
This enhanced backend specification provides a production-ready, comprehensive backup and restore system with advanced metadata tracking, real-time progress monitoring, and intelligent file management capabilities, all while maintaining AnancyIO's architectural patterns and security standards.
...
- **Testing Verified**: BackupService initializes correctly and detects AnancyIO root paths
...
**Problem:** When `include_hidden=false`, the system was excluding ALL hidden files, even when they were explicitly specified in patterns like `/a_io/.env`.

**Solution:** Implemented explicit pattern detection that distinguishes between:
- **Explicit patterns** (like `/a_io/.env`) - Always included regardless of `include_hidden` setting
- **Wildcard discoveries** (like `/a_io/*`) - Respect the `include_hidden` setting

**Result:** Critical files like `.env` are now properly backed up when explicitly specified, ensuring AnancyIO configurations are preserved.
...
The AnancyIO backup system is now production-ready and battle-tested! 🚀
...
backed_up_agent_root = environment_info.get("anancy_io_root", "")
```
...
- Works seamlessly between different AnancyIO installations
...
The AnancyIO backup system now provides:
