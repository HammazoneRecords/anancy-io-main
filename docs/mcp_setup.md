# AnancyIO: MCP Server Integration Guide

This guide explains how to configure and utilize external tool providers through the Model Context Protocol (MCP) with AnancyIO. This allows AnancyIO to leverage tools hosted by separate local or remote MCP-compliant servers.

## What are MCP Servers?

MCP servers are external processes or services that expose a set of tools that AnancyIO can use. AnancyIO acts as an MCP *client*, consuming tools made available by these servers. The integration supports three main types of MCP servers:

1.  **Local Stdio Servers**: These are typically local executables that AnancyIO communicates with via standard input/output (stdio).
2.  **Remote SSE Servers**: These are servers, often accessible over a network, that AnancyIO communicates with using Server-Sent Events (SSE), usually over HTTP/S.
...
## How AnancyIO Consumes MCP Tools

AnancyIO discovers and integrates MCP tools dynamically:

1.  **Configuration**: You define the MCP servers AnancyIO should connect to in its configuration. The primary way to do this is through the AnancyIO settings UI.
2.  **Saving Settings**: When you save your settings via the UI, AnancyIO updates the `tmp/settings.json` file, specifically the `"mcp_servers"` key.
3.  **Automatic Installation (on Restart)**: After saving your settings and restarting AnancyIO, the system will attempt to automatically install any MCP server packages defined with `command: "npx"` and the `--package` argument in their configuration (this process is managed by `initialize.py`). You can monitor the application logs (e.g., Docker logs) for details on this installation attempt.
4.  **Tool Discovery**: Upon initialization (or when settings are updated), AnancyIO connects to each configured and enabled MCP server and queries it for the list of available tools, their descriptions, and expected parameters.
...
6.  **Tool Invocation**: When the LLM decides to use an MCP tool, AnancyIO's `process_tools` method (handled by `mcp_handler.py`) identifies it as an MCP tool and routes the request to the appropriate `MCPConfig` helper, which then communicates with the designated MCP server to execute the tool.

## Configuration

### Configuration File & Method

The primary method for configuring MCP servers is through **AnancyIO's settings UI**.
...
*   **Default Value**: If `tmp/settings.json` does not exist, or if it exists but does not contain the `"mcp_servers"` key, AnancyIO will use a default value of `""` (an empty string), meaning no MCP servers are configured.
...
*   **Updating**: As mentioned, the recommended way to set or update this value is through AnancyIO's settings UI.
*   **For Existing `settings.json` Files (After an Upgrade)**: If you have an existing `tmp/settings.json` from a version of AnancyIO prior to MCP server support, the `"mcp_servers"` key will likely be missing. To add this key:
    1.  Ensure you are running a version of AnancyIO that includes MCP server support.
    2.  Run AnancyIO and open its settings UI.
...
*   `"disabled"`: A boolean (`true` or `false`). If `true`, AnancyIO will ignore this server configuration.
...
Once configured, successfully installed (if applicable, e.g., for `npx` based servers), and discovered by AnancyIO:
...
*   **Execution Flow**: AnancyIO's `process_tools` method (with logic in `python/helpers/mcp_handler.py`) prioritizes looking up the tool name in the `MCPConfig`. If found, the execution is delegated to the corresponding MCP server. If not found as an MCP tool, it then attempts to find a local/built-in tool with that name.

This setup provides a flexible way to extend AnancyIO's capabilities by integrating with various external tool providers without modifying its core codebase.
