...
> [!CAUTION]
> Preferred way of persisting AnancyIO data is to use the backup and restore feature.
> By mapping the whole `/a_io` directory to a local directory, you will run into problems when upgrading AnancyIO to a newer version.
...
- You can map individual subfolders of `/a_io` to a local directory or the full `/a_io` directory (not recommended).
...
- OPTIONAL: Under "Volumes", configure your mapped folders, if needed:
  - Example host path: Your chosen directory (e.g., `C:\anancyio\memory`)
  - Example container path: `/a_io/memory`
...
> [!CAUTION]
> **GitHub Copilot Provider:** When using the GitHub Copilot provider, after selecting the model and entering your first prompt, the OAuth login procedure will begin. You'll find the authentication code and link in the output logs. Complete the authentication process by following the provided link and entering the code, then you may continue using AnancyIO.
...
> [!NOTE]
> In settings, External Services tab, you can enable Cloudflare Tunnel to expose your AnancyIO instance to the internet.
...
> [!NOTE]
> Since v0.9, AnancyIO has a Backup and Restore feature, so you don't need to backup the files manually.
