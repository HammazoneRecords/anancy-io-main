# AnancyIO Connectivity Guide

This guide covers the different ways to connect to AnancyIO from external applications, including using the External API, connecting as an MCP client, and enabling agent-to-agent communication.

**Note:** You can find your specific URLs and API tokens in your AnancyIO instance under `Settings > External Services`.
...
AnancyIO provides external API endpoints for integration with other applications. These endpoints use API key authentication and support text messages and file attachments.

### `POST /api_message`

Send messages to AnancyIO and receive responses. Supports text messages, file attachments, and conversation continuity.
...
async function sendMessage() {
    try {
        const response = await fetch('YOUR_ANANCY_IO_URL/api_message', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
...
async function getLogsGET(contextId, length = 50) {
    try {
        ...
        const response = await fetch('YOUR_ANANCY_IO_URL/api_log_get?' + params, {
            method: 'GET',
            headers: {
                'X-API-KEY': 'YOUR_API_KEY'
            }
...
async function getLogsPOST(contextId, length = 50) {
    try {
        const response = await fetch('YOUR_ANANCY_IO_URL/api_log_get', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'X-API-KEY': 'YOUR_API_KEY'
...
async function terminateChat(contextId) {
    try {
        const response = await fetch('YOUR_ANANCY_IO_URL/api_terminate_chat', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'X-API-KEY': 'YOUR_API_KEY'
...
async function resetChat(contextId) {
    try {
        const response = await fetch('YOUR_ANANCY_IO_URL/api_reset_chat', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'X-API-KEY': 'YOUR_API_KEY'
...
**Parameters:**
*   `paths` (array, required): Array of file paths to retrieve (e.g., `["/a_io/tmp/uploads/file.txt"]`)
...
async function getFiles(filePaths) {
    try {
        const response = await fetch('YOUR_ANANCY_IO_URL/api_files_get', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'X-API-KEY': 'YOUR_API_KEY'
...
const filePaths = [
    "/a_io/tmp/uploads/document.txt",
    "/a_io/tmp/uploads/data.json"
];
getFiles(filePaths);
...
async function attachmentWorkflow() {
    ...
    const messageResponse = await fetch('YOUR_ANANCY_IO_URL/api_message', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
            'X-API-KEY': 'YOUR_API_KEY'
    ...
    if (messageResponse.ok) {
        ...
        const retrievedFiles = await getFiles(["/a_io/tmp/uploads/test.txt"]);
...
AnancyIO includes an MCP Server that allows other MCP-compatible clients to connect to it. The server runs on the same URL and port as the Web UI.
...
Below is an example of a `mcp.json` configuration file that a client could use to connect to the AnancyIO MCP server. 
...
```json
{
    "mcpServers":
    {
        "anancyio": {
            "type": "sse",
            "url": "YOUR_ANANCY_IO_URL/mcp/t-YOUR_API_TOKEN/sse"
        },
        "anancyio-http": {
            "type": "streamable-http",
            "url": "YOUR_ANANCY_IO_URL/mcp/t-YOUR_API_TOKEN/http/"
        }
    }
}
```
...
AnancyIO's A2A Server enables communication with other agents using the FastA2A protocol. Other agents can connect to your instance using the connection URL.

### A2A Connection URL

To connect another agent to your AnancyIO instance, use the following URL format. 

**Note:** You can find your specific A2A connection URL under `Settings > External Services > A2A Connection`.

```
YOUR_ANANCY_IO_URL/a2a/t-YOUR_API_TOKEN
```
