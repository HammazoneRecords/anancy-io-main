...
//     if (out.name.startsWith("data-") && out.name != "data-A_IOUID" && out.name != "data-a_io-frame-id") return null;
...
//   window.__A_IO_extractFrameContent = function() {
...
//           node.setAttribute("data-A_IOUID", uid);
//         }
...
//         if (tagName) {
//           content += ` selector="${node.getAttribute("data-A_IOUID")}"`;
//         }
...
//     if (event.data === 'A_IO_REQUEST_CONTENT') {
//       // Extract content and send it back to parent
//       const content = window.__A_IO_extractFrameContent();
//       // Use '*' as targetOrigin since we're in a controlled environment
//       window.parent.postMessage({
//         type: 'A_IO_FRAME_CONTENT',
//         content: content,
//         frameId: window.frameElement?.getAttribute('data-a_io-frame-id')
//       }, '*');
//     }
//   });

//   // Function to extract content from all frames
//   window.__A_IO_extractAllFramesContent = async function(rootNode = document) {
//     let content = "";
    
//     // Extract content from current document
//     content += window.__A_IO_extractFrameContent();
...
//         iframe.setAttribute('data-a_io-frame-id', frameId);
        
//         // Setup one-time message listener for this specific frame
//         const listener = function(event) {
//           if (event.data?.type === 'A_IO_FRAME_CONTENT' && 
...
//         iframe.contentWindow.postMessage('A_IO_REQUEST_CONTENT', '*');
...
//       const frameId = iframe.getAttribute('data-a_io-frame-id');
