...
Click to open a video to learn how to install AnancyIO:

[![Easy Installation guide](/docs/res/easy_ins_vid.png)](https://www.youtube.com/watch?v=w5v5Kjx51hs)

The following user guide provides instructions for installing and running AnancyIO using Docker, which is the primary runtime environment for the framework. For developers and contributors, we also provide instructions for setting up the [full development environment](#in-depth-guide-for-full-binaries-installation).
...
- Docker Desktop provides the runtime environment for AnancyIO, ensuring consistent behavior and security across platforms
...
- Note: AnancyIO also offers a Hacking Edition based on Kali linux with modified prompts for cybersecurity tasks. The setup is the same as the regular version, just use the anancyioai/anancyio:hacking image instead of anancyioai/anancyio.

2.1. Pull the AnancyIO Docker image:
- Search for `anancyioai/anancyio` in Docker Desktop
...
- Choose or create a directory on your machine where you want to store AnancyIO's data
- This can be any location you prefer (e.g., `C:/anancyio-data` or `/home/user/anancyio-data`)
- This directory will contain all your AnancyIO files, like the legacy root folder structure:
  ...
  - `.env` - Your API keys
  - `settings.json` - Your AnancyIO settings

> [!TIP]
> Choose a location that's easy to access and backup. All your AnancyIO data 
> will be directly accessible in this directory.

2.3. Run the container:
- In Docker Desktop, go back to the "Images" tab
- Click the `Run` button next to the `anancyioai/anancyio` image
...
- Under "Volumes", configure:
  - Host path: Your chosen directory (e.g., `C:\anancyio-data`)
  - Container path: `/a_io`
...
> Alternatively, run the following command in your terminal:
> ```bash
> docker run -p $PORT:80 -v /path/to/your/data:/a_io anancyioai/anancyio
> ```
...
- The Web UI will open. AnancyIO is ready for configuration!

![docker ui](res/setup/6-docker-a_io-running.png)

> [!TIP]
> You can also access the Web UI by clicking the ports right under the container ID in Docker Desktop.

> [!NOTE]
> After starting the container, you'll find all AnancyIO files in your chosen 
...
3. Configure AnancyIO
- Refer to the following sections for a full guide on how to configure AnancyIO.

## Settings Configuration
AnancyIO provides a comprehensive settings interface to customize various aspects of its functionality. Access the settings by clicking the "Settings"button with a gear icon in the sidebar.
...
Learn more about Remote Function Calls and their purpose [here](#7-configure-anancyio-rfc).

> [!IMPORTANT]
> Always keep your API keys and passwords secure.

# Choosing Your LLMs
The Settings page is the control center for selecting the Large Language Models (LLMs) that power AnancyIO.  You can choose different LLMs for different roles:

| LLM Role | Description |
| --- | --- |
| `chat_llm` | This is the primary LLM used for conversations and generating responses. |
| `utility_llm` | This LLM handles internal tasks like summarizing messages, managing memory, and processing internal prompts.  Using a smaller, less expensive model here can improve efficiency. |
| `embedding_llm` | This LLM is responsible for generating embeddings used for memory retrieval and knowledge base lookups. Changing the `embedding_llm` will re-index all of A_IO's memory. |
...
> mixed in the vector database. Note that this will DELETE ALL of AnancyIO's memory.
...
#### Selecting your model within AnancyIO
...
## Using AnancyIO on your mobile device
AnancyIO's Web UI is accessible from any device on your network through the Docker container:
...
> [!NOTE]
> If you're running AnancyIO directly on your system (legacy approach) instead of 
> using Docker, you'll need to configure the host manually in `run_ui.py` to run on all interfaces using `host="0.0.0.0"`.

For developers or users who need to run AnancyIO directly on their system,see the [In-Depth Guide for Full Binaries Installation](#in-depth-guide-for-full-binaries-installation).

# How to update AnancyIO

1. **If you come from the previous version of AnancyIO:**
- Your data is safely stored across various directories and files inside the AnancyIO folder.
- To update to the new Docker runtime version, you might want to backup the following files and directories:
  ...
  - `/tmp/settings.json` - Your AnancyIO settings
...
- Reach for the folder where you saved your data and copy it to the new AnancyIO folder set during the installation process.
- AnancyIO will automatically detect your saved data and use it across memory, knowledge, instruments, prompts and settings.

> [!IMPORTANT]
> If you have issues loading your settings, you can try to delete the `/tmp/settings.json` file and let AnancyIO generate a new one.
...
- Go to "Images" tab and remove the `anancyioai/anancyio` image or click the three dots to pull the difference and update the Docker image.
...
> your AnancyIO data directory (the chosen directory on your machine).
...
3. **Full Binaries**
- Using Git/GitHub: Pull the latest version of the AnancyIO repository. 
...
> - If any changes are made to the requirements of the updated version, you have to execute this command inside the a_io conda env after activating it:
> ```bash
> pip install -r requirements.txt

# In-Depth Guide for Full Binaries Installation
- AnancyIO is a framework. It's made to be customized, edited, enhanced. Therefore you need to install the necessary components to run it when downloading its full binaries. This guide will help you to do so.
- The following step by step instructions can be followed along with a video for this tutorial on how to make AnancyIO work with its full development environment.
...
2. You don't necessarily need API keys: AnancyIO can run with local models. For this tutorial though, we will leave it to the default OpenAI API. A guide for downloading Ollama along with local models is available [here](#installing-and-using-ollama-local-models).
...
6. Running AnancyIO without Docker makes the process more complicated and it's thought for developers and contributors.
...
## 2. Download AnancyIO
- You can clone the AnancyIO repository (https://github.com/anancyioai/anancyio) from GitHub if you know how to use Git. In this tutorial I will just show how to download the files.

1. Go to the AnancyIO releases [here](https://github.com/anancyioai/anancyio/releases).
2. The latest release is on the top of the list, click the "Source Code (zip)" button under "Assets" to download it.

<img src="res/setup/image-14-u.png" alt="anancyio download" width="500"/>
<br><br>

3. Extract the downloaded archive where you want to have it. I will extract them to "anancyio" folder on my Desktop - "C:\Users\frdel\Desktop\anancyio" on Windows and "/Users/frdel/Desktop/anancyio" on macOS.
...
2. In the terminal, navigate to your AnancyIO folder using **"cd"** command. Replace the path with your actual AnancyIO folder path.
~~~
cd C:\Users\frdel\Desktop\anancyio
~~~
You should see your folder has changed on the next terminal line.

<img src="res/setup/image-15.png" alt="anancyio cd" height="100"/>
<img src="res/setup/image-16.png" alt="anancyio cd" height="100"/>
<br><br>

3. Create Conda environment using command **"conda create"**. After **"-n"** is your environment name, you can choose your own, i will use **"a_io"** - short for AnancyIO. After **"python"** is the Python version that Conda will install for you into this environment, right now, 3.12 works fine. **-y** skips confirmations.
~~~
conda create -n a_io python=3.12 -y
~~~
...
conda activate a_io
~~~
And you should see that the **(base)** on the left has changed to **(a_io)**. This means that this terminal now uses the new **a_io** virtual environment and all packages will be installed into this environment.
...
> "conda activate a_io" again for that window.
...
Simply put, Docker is a way of running virtual computers on your machine. These are lightweight, disposable and isolated from your operating system, so it is a way to sandbox AnancyIO.
- AnancyIO only connects to the Docker container when it needs to execute code and commands. The frameworks itself runs on your machine.
...
> Docker socket to be used (requires password)."  This allows AnancyIO to 
...
- AnancyIO needs a Docker image to be pulled from the Docker Hub to be run, even when using the full binaries.
You can refer to the [installation instructions above](#windows-macos-and-linux-setup-guide) to run the Docker container and then resume from the next step. There are two differences:
  ...
  - You need to map the `/a_io` volume to the location of your local AnancyIO folder.
- Run the Docker container following the instructions.

## 5. Run the local AnancyIO instance
Run the AnancyIO with Web UI:
~~~
python run_ui.py
~~~
...
- Open the URL shown in terminal in your web browser. You should see the AnancyIO interface.

## 6. Configure AnancyIO
Now we can configure AnancyIO - select models, settings, API Keys etc. Refer to the [Usage](usage.md#agent-configuration) guide for a full guide on how to configure AnancyIO.

## 7. Configure AnancyIO RFC
AnancyIO needs to be configured further to redirect some functions to the Docker container. This is crucial for development as A_IO needs to run in a standardized environment to support all features.
...
9. Now you have the full development environment to work on AnancyIO.
...
### Conclusion
After following the instructions for your specific operating system, you should have AnancyIO successfully installed and running. You can now start exploring the framework's capabilities and experimenting with creating your own intelligent agents. 

If you encounter any issues during the installation process, please consult the [Troubleshooting section](troubleshooting.md) of this documentation or refer to the AnancyIO [Skool](https://www.skool.com/anancyio) or [Discord](https://discord.gg/B8KZKNsPpj) community for assistance.
