...
def fix_dev_path(path: str):
    "On dev environment, convert /a_io/... paths to local absolute paths"
    from python.helpers.runtime import is_development

    if is_development():
        if path.startswith("/a_io/"):
            path = path.replace("/a_io/", "")
    return get_abs_path(path)


def normalize_a_io_path(path: str):
    "Convert absolute paths into /a_io/... paths"
