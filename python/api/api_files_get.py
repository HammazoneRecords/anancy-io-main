        ...
        try:
            ...
            for path in paths:
                try:
                    ...
                    if path.startswith("/a_io/tmp/uploads/"):
                        # Internal path - convert to external
                        filename = path.replace("/a_io/tmp/uploads/", "")
                        external_path = files.get_abs_path("tmp/uploads", filename)
                        filename = os.path.basename(external_path)
                    elif path.startswith("/a_io/"):
                        # Other internal AnancyIO paths
                        relative_path = path.replace("/a_io/", "")
