...
class LiteLLMChatWrapper(SimpleChatModel):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        # Set A_IO model config as instance attribute after parent init
        self.a_io_model_conf = model_config
    ...
    def _call(
        ...
        **kwargs: Any,
        ...
        apply_rate_limiter_sync(self.a_io_model_conf, str(msgs))
    ...
    async def _astream(
        ...
        **kwargs: Any,
        ...
        # Apply rate limiting if configured
        await apply_rate_limiter(self.a_io_model_conf, str(msgs))
    ...
    async def unified_call(
        ...
        **kwargs: Any,
        ...
        limiter = await apply_rate_limiter(
            self.a_io_model_conf, str(msgs_conv), rate_limiter_callback
        )

        # Prepare call kwargs and retry config (strip A_IO-only params before calling LiteLLM)
        call_kwargs: dict[str, Any] = {**self.kwargs, **kwargs}
        max_retries: int = int(call_kwargs.pop("a_io_retry_attempts", 2))
        retry_delay_s: float = float(call_kwargs.pop("a_io_retry_delay_seconds", 1.5))
...
class BrowserCompatibleChatWrapper(ChatOpenRouter):
    ...
    async def _acall(
        ...
        **kwargs: Any,
        ...
        # Apply rate limiting if configured
        apply_rate_limiter_sync(self._wrapper.a_io_model_conf, str(messages))
...
class LiteLLMEmbeddingWrapper(Embeddings):
    model_name: str
    kwargs: dict = {}
    a_io_model_conf: Optional[ModelConfig] = None
    ...
    def embed_documents(self, texts: List[str]) -> List[List[float]]:
        # Apply rate limiting if configured
        apply_rate_limiter_sync(self.a_io_model_conf, " ".join(texts))
    ...
    def embed_query(self, text: str) -> List[float]:
        # Apply rate limiting if configured
        apply_rate_limiter_sync(self.a_io_model_conf, text)
...
def _adjust_call_args(provider_name: str, model_name: str, kwargs: dict):
    ...
    if provider_name == "openrouter":
        kwargs["extra_headers"] = {
            "HTTP-Referer": "https://anancyio.ai",
            "X-Title": "AnancyIO",
        }
